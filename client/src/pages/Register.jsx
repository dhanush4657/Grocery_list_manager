...
const Register = () => {
  ...
  const handleVerify = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${API}/verify-email`, { email: formData.email, otp });
  ...
  const handleResend = async () => {
    ...
    setError("");
    try {
      await axios.post(`${API}/resend-otp`, { email: formData.email, purpose: "verification" });
      resetCountdown();
      setOtp("");
    } catch (err) {
        ...
        <Card className="border-slate-200 shadow-sm">
          ...
          <CardFooter className="pt-4">
            <p className="text-center w-full text-xs text-slate-500">
              ...
              <Link to="/" className="text-slate-900 font-medium hover:underline">Sign in</Link>
            </p>
          </CardFooter>
        </Card>
      </div>
